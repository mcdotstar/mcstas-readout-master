...
hid_t build_compound_hid(const TypeSchema& schema) {
  const hid_t compound_tid = H5Tcreate(H5T_COMPOUND, schema.total_size);
  if (compound_tid < 0) {
  ...
  for (const auto& field : schema.fields) {
    ...
    if (field.array_count > 0) {
      ...
      const hid_t array_tid = H5Tarray_create2(member_tid, 1, &dims);
      if (array_tid < 0) {
...
void write_hdf5_c_api(const hid_t file_id, const std::string& dataset_name,
                       const TypeSchema& schema, const uint8_t* data, const size_t count) {
  const hid_t compound_tid = build_compound_hid(schema);
  ...
  const hid_t dataset = H5Dcreate2(file_id, dataset_name.c_str(), compound_tid, dataspace,
  ...
  const herr_t status = H5Dwrite(dataset, compound_tid, H5S_ALL, H5S_ALL, H5P_DEFAULT, data);
  H5Dclose(dataset);
  H5Sclose(dataspace);
  H5Tclose(compound_tid);
...
// Mode 2: Opaque
CollectorStar::CollectorStar(const size_t object_size, const std::string& dataset_name)
...
// Mode 3: Validated
CollectorStar::CollectorStar(const std::string& type_description, const size_t object_size, const std::string& dataset_name)
...
void CollectorStar::get(const size_t index, void* dst) const {
  if (index >= count_) {
...
void CollectorStar::write_hdf5(const std::string& filename) const {
  const HighFive::File file(filename, HighFive::File::OpenOrCreate);
  write_hdf5(file);
}

void CollectorStar::write_hdf5(const HighFive::File& file) const {
  auto root = file.getGroup("/");
  write_hdf5(root);
}

void CollectorStar::write_hdf5(HighFive::Group& group) const {
  if (count_ == 0) return;
  ...
  if (has_schema()) {
    if (schema_has_arrays(*schema_)) {
      ...
      write_hdf5_c_api(group.getId(), dataset_name_, *schema_, data_.data(), count_);
    } else {
      const auto compound_type = build_hdf5_compound_type(*schema_);
      const auto dataspace = HighFive::DataSpace({count_});
      auto dataset = group.createDataSet(dataset_name_, dataspace, compound_type);
      dataset.write_raw(data_.data(), compound_type);
    }
  } else {
    // Opaque mode: write as a 2D array of uint8_t [count_ x object_size_]
    const auto dataspace = HighFive::DataSpace({count_, object_size_});
    auto dataset = group.createDataSet(dataset_name_, dataspace, HighFive::create_datatype<uint8_t>());
